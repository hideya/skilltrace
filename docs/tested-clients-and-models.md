...
version, provider-recorded model, instruction profile, and validation scope. It
is a compatibility ledger, not a model benchmark or a claim that one successful
run proves general behavior.

Provider-recorded identity is preferred when available. Agent-declared identity
is not used here unless it is explicitly marked uncertain. Client-owned history
formats can change independently of the model, so observed behavioral
differences should be treated as clues for skill debugging rather than causal
model comparisons.
...
| Level | Meaning |
| --- | --- |
| End-to-end | `skilltrace start`, agent work, MCP/passive tracing, `skilltrace stop`, provider-history collection, storage, and UI inspection were exercised |
| Provider-history | Matching, privacy projection, normalized events, collection summary, and UI identity were exercised against a real session |
| Format inspection | A real provider file was inspected safely, but the complete current workflow was not validated for that combination |

## Supported Workflow Matrix

| Client or agent system | Instruction profile | MCP workflow | Provider-history adapter | Current status |
| --- | --- | --- | --- | --- |
...
| Codex Desktop/App | Not established as a supported workflow | Not validated end-to-end | Codex rollout records recognized | Provider-history observation only; Codex App remains unsupported |
...
caused the format change; a concurrent provider protocol rollout is also
plausible.
...
- It does not treat task success as proof that the intended skill caused the
  result.
- It does not treat agent-declared model names as authoritative when provider
  history disagrees.
- It does not imply support for Codex App merely because Codex Desktop rollout
  records can be parsed.
- It does not guarantee future compatibility with provider-owned history
  formats.

## Update Policy

Add or revise a row when a materially distinct client version, provider-recorded
model, instruction profile, platform, or history envelope is exercised.

For each row:

1. Prefer provider-recorded client and model identity.
...
Detailed provider-owned formats and privacy dispositions remain in
[Provider History Formats](./provider-history-formats.md). The repeatable demo
