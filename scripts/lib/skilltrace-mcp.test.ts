...
describe('skilltrace MCP helpers', () => {
  ...
  test('builds a run reflection event from MCP input', () => {
    let event = buildMcpSkillTraceReflectionEvent(
      {
        ...
        skills_read: ['.agents/skills/type-fix/SKILL.md'],
        references_read: ['.agents/skills/type-fix/references/checklist.md'],
    ...
    expect(event.data.skills_read).toEqual(['.agents/skills/type-fix/SKILL.md'])
